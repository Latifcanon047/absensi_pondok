...
export async function GET(request: NextRequest) {
  try {
    ...
    const data = await prisma.absenKelas.findMany({
      where: { absensiId },
    });

    return NextResponse.json(data);
  } catch (error) {
    console.error("GET absen-kelas error:", error);
    return NextResponse.json(
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.error("POST absen-kelas error:", error);
    return NextResponse.json(
