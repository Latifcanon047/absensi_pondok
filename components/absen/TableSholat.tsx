...
export default function TabelSholat({
  santriList,
  sholatStates,
  submitting,
  sukses,
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 mb-6 overflow-hidden hover:shadow-md transition-all duration-300">
      ...
      <div className="px-6 py-4 border-b border-gray-100 bg-linear-to-r from-gray-50 to-white flex justify-between items-center">
        <div>
          <h2 className="font-semibold text-gray-800">Absen Sholat</h2>
                ...
                {WAKTU_SHOLAT.map((waktu) => (
                  <td key={waktu} className="px-4 py-3 text-center">
                    <StatusPicker
                      ...
                      onChange={(s) => onStatusChange(santri.id, waktu, s)}
                      disabled={submitting !== null}
                    />
                  </td>
                ))}
